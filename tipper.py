...
def withdraw_final(message):
    if message.text == "🚫 Cancel":
        ...
        try:
            ...
            bot.send_message(message.chat.id, "<b>Making Withdraw now</b>\n\n<b>Amount : </b>" + str(amu) +"\n<b>Address : </b>"+str(addre)+"\n<b>Currency : </b>"+str(currr) + "\n<b>Txid : </b>" + str(txid), parse_mode="html", reply_markup=menu())
        except Exception as e:
    ...
    if float(amount) <= float(user_doge_bal(chatids)):
        if float(percent) < 100:
            if float(amount) > 0.9:
                ...
                if op == "hi":
                    if float(ran_per) > float(percent):
                        perrr = (float(percent)/100)
                        #print(float(perrr))
                        amu = float(amount) + 0.0000
                        #print(amu)
                ...
                if op == "lo":
                    if float(ran_per) < float(percent):
                        percen = 100 - float(percent)
                        perrr = (float(percen)/100)
                ...
                bot.reply_to(message, "You must select a percent between 1 to 99")
        else:
